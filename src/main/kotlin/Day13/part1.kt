...
private val packets = File("src/main/kotlin/Day13/input1.txt").readLines().filter { it.isNotBlank() }.map { Packet.of(it) }
...
private sealed class Packet : Comparable<Packet> {
    companion object {
        fun of(input: String): Packet =
            of(
                input.split("""((?<=[\[\],])|(?=[\[\],]))""".toRegex()) //fucking Regex but it works i guess
